...
if __name__ == "__main__":
    ...
    select = "select sum(count) as counts,province from ha_ip_%s group by province order by counts desc"%(timeStr)
    data = select_mysql(select)

    for i in data:
        insert="insert into app01_the_number_of_clicks values (null,'%s','%s','%s')"%(i[1],i[0],datestrftime)
        insert_mysql(insert)
        print(insert)